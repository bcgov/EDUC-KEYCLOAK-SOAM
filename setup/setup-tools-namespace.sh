...
SONAR_TOKEN_CODETABLE_API = $(curl --silent -u admin:admin -X POST --data "name=TEST" "https://sonarqube-c2mvws-tools.pathfinder.gov.bc.ca/api/user_tokens/generate" | jq '.token')

echo Creating Code Table API secrets
